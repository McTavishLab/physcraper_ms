...
hypothesis based on already available expert phylogentic knowledge.
...
about phylogenetic methods but do not know much acbout a certain biological group.***
...
However, automatizating the assembly of the character data set is a crucial step towards
...
## Study tree and alignment

- The phylogenetic tree has to be in Open Tree of Life. You can choose form a variety of ytrees supporting any node of the Tree of Life.
- The alignment should be a gene alignment that was used to generate the tree. The alignments are usually stored in treebase or dryad.
  If it is stored in Treebase, it can be downloaded by pyscraper. If it is on dryad, it has to be provided by the user.
- Taxon name matching step: check that taxon names on the tips of the trees are in the character matrix and visceversa.
- A csv file with the summary of taxon name matching is produced for the user.
- Unmatched taxon names are dropped from the tree and alignment. ***What is the minimum amount of tips necessary for a physcraper run??***
- A .tre and .aln files are generated for a physcraper run.

## DNA sequence search and cleaning

- The first step is to identify the taxonomic MRCA of all species in the tree. The taxonomic MRCA
  is the closest phylogenetic MRCA that has a taxonomic name assigned in the NCBI database. ***Does
  physcraper take into account the focal clade from Open Tree in any way???***
...
- A gene tree is reconstructed for each alignment provided, using ***what software?***
...
the synthetic OToL and then measure how well they do agianst each other***
