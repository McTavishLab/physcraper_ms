...
Updating existing phylogenies with publicly available molecular sequence data provides the opportunity to study
...
%% Maybe show conflict results? and then discuss in the discussion.
...
This ``seqlen\_mismatch.txt'' file includes the accession number, taxon, and sequence length of all sequences culled based on sequence length. 
...
Multiple sequence alignment of loci of drastically different lengths is unfeasible, and we have found in testing often returns incorrect results, splitting shorter sequences with many long gaps to align with exact matches across the entire longer locus. 
...
Instead we list the accession numbers for these matches in the seqlen\_mismatch file, for users to assess appropriate homologous regions to their alignment of interest.
...
The Physcraper workflow avoids some of these challenges, by focussing on ingroup taxa of an exisiting phylogeny, using markers that have been assessed and proven apporpraite for that phylogenetic scope in past publictions.
In addition, we thin alignments by removing identical sequences, and by setting a maximum number of sequences per taxon.
Nonetheless, it is encumbent on users to assess their final inference with respect to statistical support and biological plausibility.
...
Once sequences are aligned, they can apply and compare inferences from any phylogenetics software.
...
We have included a script (``multi_locus.py'') and examples of how to automatically merge the outputs of Physcraper runs at different loci into input files for ASTRAL, SVD Quartets, or as concatenated alignments for concatenated analyses.
...
These taxonomic links, and comparisons to existing published phylogenies in the OpenTree data store can also help flag paralogous sequences.
