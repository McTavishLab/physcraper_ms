...
%DIF DEL sources/bmc_template_submission/bmc_article_submission_2021-01-08.tex   Wed Apr 28 17:22:44 2021
%DIF ADD sources/bmc_template_submission/bmc_article.tex                         Wed Apr 28 22:22:50 2021
%%                                      %
...
}\DIFaddend with public DNA data \DIFaddbegin \DIFadd{from GenBank \mbox{%DIFAUXCMD
\cite{benson2000genbank}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , and links the tips in \DIFdelbegin \DIFdel{these }\DIFdelend \DIFaddbegin \DIFadd{the updated }\DIFaddend trees to a unified, interoperable
...
By using the existing OpenTree taxonomy programmatic tools to
map tip names ,
...
that are open as services for use by the general public \mbox{%DIFAUXCMD
...
}\DIFaddend a framework for \DIFdelbegin \DIFdel{connecting }\DIFdelend \DIFaddbegin \DIFadd{automatic connection of }\DIFaddend updated phylogenies with data from any biological database.
...
\DIFdelbegin \DIFdel{OpenTree's comprehensive }\DIFdelend \DIFaddbegin \DIFadd{Using its unified taxonomy, OpenTree assembles a comprehensive synthetic }\DIFaddend tree of life \DIFdelbegin \DIFdel{comprises }\DIFdelend \DIFaddbegin \DIFadd{comprising
}\DIFaddend 2.3 million tips, of which around \DIFaddbegin \DIFadd{only
}\DIFaddend 90,000 are supported by \DIFdelbegin \DIFdel{phylogenies }\DIFdelend \DIFaddbegin \DIFadd{phylogenetic data uploaded to OpenTrees' database (the Phylesystem
\mbox{%DIFAUXCMD
\cite{mctavish2015phylesystem}}\hspace{0pt}%DIFAUXCMD
) by curators }\DIFaddend - the remaining 1.4 million taxa are placed
...
but that is a \DIFdelbegin \DIFdel{time consuming }\DIFdelend \DIFaddbegin \DIFadd{time-consuming }\DIFaddend approach which is \DIFdelbegin \DIFdel{not highly reproducible.
A variety of }\DIFdelend \DIFaddbegin \DIFadd{largely not reproducible.
...
widely }\DIFaddend non-overlapping homology hypotheses across taxa \cite{jones2016targeted},
...
update and compare \DIFdelbegin \DIFdel{existing phylogenetic }\DIFdelend \DIFaddbegin \DIFadd{new phylogenetic hypotheses to existing }\DIFaddend knowledge.
...
link any phylogeny mapped to OpenTree's standardized taxonomy\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{mctavish2015phylesystem}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend ,
...
Taxon names in the input tree must be standardized to \DIFdelbegin \DIFdel{OpenTree taxonomy
\mbox{%DIFAUXCMD
\cite{rees2017automated} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{the OpenTree taxonomy
\mbox{%DIFAUXCMD
...
search taxon is identified using OpenTree's taxonomic API \mbox{%DIFAUXCMD
...
sequence in the input alignment and they represent the same taxon in OpenTree\DIFaddbegin \DIFadd{'s }\DIFaddend or
NCBI taxonomy.
...
and 500 accepted living species \mbox{%DIFAUXCMD
\cite{plantlist}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , and is
the only extant \DIFdelbegin \DIFdel{clade }\DIFdelend \DIFaddbegin \DIFadd{taxon }\DIFaddend within the family Aquifoliaceae, order Aquifoliales of flowering
...
selbach2009new}, but only two have data publicly available\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{gottlieb2005molecular, yao2020phylogeny}}\hspace{0pt}%DIFAUXCMD
.
\mbox{%DIFAUXCMD
\cite{gottlieb2005molecular} }\hspace{0pt}%DIFAUXCMD
made original tree and alignment }\DIFdelend \DIFaddbegin \DIFadd{, the ``Gottlieb2005" study
\mbox{%DIFAUXCMD
\cite{gottlieb2005molecular} }\hspace{0pt}%DIFAUXCMD
and the ``Yao2020" study \mbox{%DIFAUXCMD
\cite{yao2020phylogeny}}\hspace{0pt}%DIFAUXCMD
.
Gottlieb2005 made their original published phylogeny and alignment openly }\DIFaddend available in TreeBASE
(study 1091 \cite{treebase1091}). The \DIFdelbegin \DIFdel{tree sampling }\DIFdelend \DIFaddbegin \DIFadd{Gottlieb2005 phylogeny samples }\DIFaddend 41 species\DIFdelbegin \DIFdel{is also }\DIFdelend \DIFaddbegin \DIFadd{, is }\DIFaddend available from
...
and alignment to update relationships in the genus }\textot{\DIFadd{Ilex}}\DIFadd{,
...
, }\DIFdelend or SVD Quartets \cite{chifman2014quartet}\DIFdelbegin \DIFdel{).
Rigorous analyses of multiple loci allows for more complex evolutionary models
than analysesof large genomic data sets, and can provide better evolutionary estimates. }\DIFdelend \DIFaddbegin \DIFadd{.
...
\DIFaddbegin \DIFadd{Our example application of Physcraper to update a holly phylogeny is based on a
...
OpenTree's conflict analysis tool informs the users of whether their tree contains major
...
constructed using taxonomy of the genus
  ...
  Gottlieb2005 phylogeny}\DIFaddendFL .
}
\end{figure}
