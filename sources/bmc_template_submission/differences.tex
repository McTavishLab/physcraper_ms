...
%DIF DEL sources/bmc_template_submission/bmc_article_submission_2021-01-08.tex   Tue Mar  2 14:52:09 2021
%DIF ADD sources/bmc_template_submission/bmc_article.tex                         Fri Apr 23 19:07:28 2021
%%                                      %
...
framework for biological databases to automate data connections across databases\DIFdelbegin \DIFdel{,
with the main goal of building on }\DIFdelend \DIFaddbegin \DIFadd{.
Physcraper's main goal is to build upon }\DIFaddend published alignments and \DIFdelbegin \DIFdel{extending }\DIFdelend \DIFaddbegin \DIFadd{published phylogenies
to extend }\DIFaddend existing phylogenetic inferences with more data and taxa\DIFaddbegin \DIFadd{, which improve
phylogenetic reconstruction \mbox{%DIFAUXCMD
...
,
biogeographic analyses \mbox{%DIFAUXCMD
\cite{kayaalp2017back}}\hspace{0pt}%DIFAUXCMD
,
as well as help in resolving phylogenetic conflict \mbox{%DIFAUXCMD
\cite{hedtke2006resolution, townsend2010optimal, natsidis2019phylogenomics}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
Physcraper updates a starting \DIFdelbegin \DIFdel{tree and single locus alignments
}\DIFdelend \DIFaddbegin \DIFadd{single locus alignment and corresponding tree
}\DIFaddend with public DNA data, and links the tips in \DIFdelbegin \DIFdel{these }\DIFdelend \DIFaddbegin \DIFadd{the updated }\DIFaddend trees to a unified, interoperable
...
\DIFdelbegin \DIFdel{OpenTree's comprehensive }\DIFdelend \DIFaddbegin \DIFadd{The Open Tree of Life project assembles a comprehensive synthetic }\DIFaddend tree of life \DIFdelbegin \DIFdel{comprises }\DIFdelend \DIFaddbegin \DIFadd{comprising
}\DIFaddend 2.3 million tips, of which around \DIFaddbegin \DIFadd{only
}\DIFaddend 90,000 are supported by \DIFdelbegin \DIFdel{phylogenies }\DIFdelend \DIFaddbegin \DIFadd{phylogenetic data uploaded to OpenTrees' database (the Phylesystem) by curators }\DIFaddend - the remaining 1.4 million taxa are placed
...
as a Python script or using the command line interface we developped for it.
...
\DIFadd{We improved code syntax of Physcraper's modules with low Pylint scores, and fixed code errors by
...
%DIF >  Pylint score for each module and locally disabled Pylint reccomendations
...
gaps of all sequences in input alignment (default values of 80%DIF <  and 120%,
...
An online literature review in June 2020 (Google scholar search for \DIFdelbegin \DIFdel{"}\DIFdelend \DIFaddbegin \DIFadd{``}\DIFaddend ilex phylogeny")
...
(study 1091 \cite{treebase1091}). \DIFdelbegin \DIFdel{The tree sampling }\DIFdelend \DIFaddbegin \DIFadd{This tree (Gottlieb2005 tree from now on) samples }\DIFaddend 41 species\DIFdelbegin \DIFdel{is also }\DIFdelend \DIFaddbegin \DIFadd{, is }\DIFaddend available from
...
The most recent \textit{Ilex} tree \cite{yao2020phylogeny} \DIFaddbegin \DIFadd{(Yao2020 tree from now on) }\DIFaddend is available in
...
tree }\DIFdelend \DIFaddbegin \DIFadd{Yao2020 tree \mbox{%DIFAUXCMD
\cite{yao2020phylogeny} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend is the best sampled phylogeny \DIFdelbegin \DIFdel{yet }\DIFdelend available for
the \DIFdelbegin \DIFdel{hollies.
}\DIFdelend \DIFaddbegin \DIFadd{genus }\textit{\DIFadd{Ilex}}\DIFadd{.
...
spacer DNA region (ITS) alignment \DIFaddbegin \DIFadd{that was used to construct the tree }\DIFaddend from \cite{gottlieb2005molecular},
...
tree }\DIFdelend \DIFaddbegin \DIFadd{Gottlieb2005 tree \mbox{%DIFAUXCMD
\cite{gottlieb2005molecular} }\hspace{0pt}%DIFAUXCMD
updated using Physcraper }\DIFaddend (Figure 2\DIFaddbegin \DIFadd{;
Physcraper updated tree from now on}\DIFaddend ) displays all 41 distinct
taxa from the original study plus 231 new tips, contributing phylogenetic data to
84 additional \textit{Ilex} taxa. The best \DIFdelbegin \DIFdel{RaxML tree }\DIFdelend \DIFaddbegin \DIFadd{ML tree from the RAxML analysis }\DIFaddend is 99\%
...
(Figure 2).
\DIFdelbegin \DIFdel{For comparison , }\DIFdelend \DIFaddbegin \DIFadd{As comparison with the Physcraper updated tree, the Yao2020 tree }\DIFaddend \cite{yao2020phylogeny}
...
\cite{gottlieb2005molecular}}\hspace{0pt}%DIFAUXCMD
study,
and contributes }\DIFdelend \DIFaddbegin \DIFadd{sampled in the Gottlieb2005 tree \mbox{%DIFAUXCMD
\cite{gottlieb2005molecular}}\hspace{0pt}%DIFAUXCMD
,
...
\cite{yao2020phylogeny} }\hspace{0pt}%DIFAUXCMD
also used
}\DIFdelend \DIFaddbegin \DIFadd{the Physcraper updated tree.
While the Yao2020 tree \mbox{%DIFAUXCMD
\cite{yao2020phylogeny} }\hspace{0pt}%DIFAUXCMD
...
However, }\DIFdelend \DIFaddbegin \DIFadd{taxa that are only on the Yao2020 tree because the DNA data is unavailable.
We also note that Physcraper incorporates 18 }\textit{\DIFadd{Ilex}} \DIFadd{taxa that are not in the Yao2020
tree \mbox{%DIFAUXCMD
\cite{yao2020phylogeny}}\hspace{0pt}%DIFAUXCMD
. These taxa appear nested among other }\textit{\DIFadd{Ilex}} \DIFadd{species
and visual inspection of the DNA sequences suggests they are correctly assigned as }\textit{\DIFadd{Ilex}}\DIFadd{.
The ITS alignment that underlies the Yao2020 tree was constructed without any tool to
scrape GenBank \mbox{%DIFAUXCMD
\cite{yao2020phylogeny}}\hspace{0pt}%DIFAUXCMD
...
\cite{yao2020phylogeny}}\hspace{0pt}%DIFAUXCMD
.
}\DIFdelend \DIFaddbegin \DIFadd{additional }\textit{\DIFadd{Ilex}} \DIFadd{taxa in the updated tree (Figure 2).
}\DIFaddend 

%DIF > % Maybe show conflict results? and then discuss in the discussion.
\DIFaddbegin 

\subsection*{\DIFadd{Verification test}}

\DIFadd{To test the accuracy of Physcraper we designed a verification test.
We pruned 9 out of the 41 original tips of the Gottlieb2005 tree \mbox{%DIFAUXCMD
\cite{gottlieb2005molecular}}\hspace{0pt}%DIFAUXCMD
,
...
set at hand.
There is not currently strong support in the literature for any particular cutoffs
...
be excluded from automatic addition based on length cutoffs.
...
match would exclude potentially homologous flanking regions which are not matched
by blasts local search algorithm, but may be important for phylogenetic inference.
Instead we list the accession numbers for these matches in the ``seqlen\_mismatch.txt"
file, for users to assess appropriate homologous regions for their alignment of interest.
}


\DIFadd{Single gene phylogenies with very high numbers of taxa may lack sufficient signal
for accurate resolution \mbox{%DIFAUXCMD
\cite{morel_phylogenetic_2020}}\hspace{0pt}%DIFAUXCMD
.
The Physcraper workflow avoids some of these challenges, by focussing on ingroup
taxa of an existing phylogeny, using markers that have been assessed and proven
apporpraite for that phylogenetic scope in past publictions.
In addition, Physcraper thins alignments by removing identical sequences, and by
setting a maximum number of sequences per taxon.
...
, }\DIFdelend or SVD Quartets \cite{chifman2014quartet}\DIFdelbegin \DIFdel{)}\DIFdelend .
...
\DIFaddbegin \DIFadd{For example \mbox{%DIFAUXCMD
...
merge the outputs of Physcraper runs at different loci into input files for ASTRAL,
SVD Quartets, or as concatenated alignments for concatenated analyses.
}


\DIFaddend Physcraper has the added advantage of facilitating the linkage of taxonomic information about tips
...
This conflict tool also returns information on if these taxonomic conflicts existed
in the original input tree.
These conflicts may be a sign that taxonomy needs to be updated, or may be a sign
that non-homologous sequences have been included in the analysis.
These taxonomic and phylogenetic conflicts flag regions for the tree for the researcher
to more closely examine and assess homology.
}\DIFaddend 


%DIF > % Biological plausibility of example??
\DIFaddbegin \DIFadd{The updated holly tree is based on a single marker, so we expect for it to be
...
note that samples belonging to the same }\textit{\DIFadd{Ilex}} \DIFadd{species also cluster together forming
monophyletic groups. A notable exception is }\textit{\DIFadd{Ilex theeizans}}\DIFadd{, which appears as
non-monophyletic in the updated tree and the original Gottlieb et al. 2005 tree.
%DIF > This suggests that the samples might represent two ifferent species, but further
analyses should be conducted
A visual comparison of the Yao et al. 2020 tree and the original Gottlieb et al. 2005 tree suggests
that the relationships within the genus }\textit{\DIFadd{Ilex}} \DIFadd{are still being determined, and that
increased taxon sampling is helping with it.
}

\DIFaddend The Physcraper workflow can be used to rapidly (in a matter of hours) create
...
comments on this manuscript. \DIFaddbegin \DIFadd{We thank the valuable comments of David Posada, Rutger Vos,
