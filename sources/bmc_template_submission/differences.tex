...
%DIF DEL sources/bmc_template_submission/bmc_article_submission_2021-05-04.tex   Fri May 28 13:49:33 2021
%DIF ADD sources/bmc_template_submission/bmc_article.tex                         Fri May 28 16:14:10 2021
%%                                      %
...
\DIFdelbegin %DIFDELCMD < \author[
%DIFDELCMD <   addressref={aff1},
%DIFDELCMD <   corref={aff1},
%DIFDELCMD <   email={ejmctavish@ucmerced.edu}
%DIFDELCMD < ]{%%%
\DIFdelend \DIFaddbegin \author[
  addressref={aff1},
  % corref={aff1},
  email={ejmctavish@ucmerced.edu}
]{\DIFaddend \inits{E.J.}\fnm{Emily Jane} \snm{McTavish}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%                                          %%
...
across the tree of life. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend We believe this is a key step to successfully establish an open, reproducible workflow for phylogenetics\DIFdelbegin \DIFdel{,
facilitating }\DIFdelend \DIFaddbegin \DIFadd{.
As such, it facilitates access to }\DIFaddend phylogenetic knowledge for \DIFdelbegin \DIFdel{ecologists and other }\DIFdelend non-specialists \DIFdelbegin \DIFdel{, effectively
democratizing }\DIFdelend \DIFaddbegin \DIFadd{in diverse research areas (e.g., ecology, medicine),
effectively contributing to the democratization of }\DIFaddend phylogenetic studies.
...
public molecular data available. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Several analysis }\DIFdelend \DIFaddbegin \DIFadd{Unlike phylogenetic placement approaches \mbox{%DIFAUXCMD
\cite{berger_performance_2011, matsen_pplacer_2010}}\hspace{0pt}%DIFAUXCMD
,
...
The tool PUMPER \mbox{%DIFAUXCMD
\cite{izquierdo2014pumper} }\hspace{0pt}%DIFAUXCMD
shares these conceptual strengths, but is no
longer under active development, is challenging to install and run, and has produced
few published phylogenetic analyses since its publication.
Several other existing }\DIFaddend tools create full phylogenies \textit{de novo} by mining of molecular
databases \cite{antonelli2017toward, sanderson2008phylota, smith2019pyphlawd,
bennett2018phylotar, pearse2013phylogenerator}. In particular, Phylota \cite{sanderson2008phylota},
and PHLAWD \cite{smith2009mega}, have been cited and used abundantly.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Physcraper builds on }\DIFdelend \DIFaddbegin \DIFadd{Physcraper adds to }\DIFaddend this automated database mining concept by incorporating prior
phylogenetic work and existing taxonomic domain knowledge on appropriate markers
and alignment construction\DIFaddbegin \DIFadd{, to update existing phylogenetic knowledge}\DIFaddend .
This decreases error (requiring less manual downstream processing) and
eases comparison with previous phylogenetic \DIFdelbegin \DIFdel{knowledge.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{hypotheses.
}\DIFaddend Results from the verification test highlight the importance of incorporating existing expertly
...
sequence length cutoff parameter, based on \DIFdelbegin \DIFdel{the filtered sequences.
Default }\DIFdelend \DIFaddbegin \DIFadd{said information.
Physcraper's default }\DIFaddend filtering parameters are arbitrary, but we hope that by making the process
...
\DIFaddbegin \DIFadd{filtering }\DIFaddend choices have on their inferences.
This is in contrast to ``manual" searches for taxa, where similarly arbitrary filters
are applied, but are difficult to trace.
\DIFdelbegin \DIFdel{As many studies have shown \mbox{%DIFAUXCMD
\cite{huang_what_2009} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{It has been shown \mbox{%DIFAUXCMD
\cite{huang_what_2009} }\hspace{0pt}%DIFAUXCMD
that }\DIFaddend the effect of missing data \DIFaddbegin \DIFadd{in alignments }\DIFaddend can
be enigmatic, and interact with the true phylogenetic relationships for the \DIFdelbegin \DIFdel{data
set }\DIFdelend \DIFaddbegin \DIFadd{dataset }\DIFaddend at hand.
There is not currently strong support in the literature for any particular \DIFaddbegin \DIFadd{filtering }\DIFaddend cutoff
value, and rather than prescribe specific approaches, we encourage users to explore
the effects of different \DIFdelbegin \DIFdel{choices on their phylogenetic inferences}\DIFdelend \DIFaddbegin \DIFadd{parameter values on resulting phylogenetic hypotheses}\DIFaddend .
In addition, by providing the output files at each step of the analysis,
it is straightforward to assess how changing \DIFdelbegin \DIFdel{parameter }\DIFdelend \DIFaddbegin \DIFadd{filtering cutoffs }\DIFaddend and software choices \DIFdelbegin \DIFdel{do or
do not }\DIFdelend \DIFaddbegin \DIFadd{might }\DIFaddend drive differences in phylogenetic inference.
By gathering the \DIFaddbegin \DIFadd{DNA }\DIFaddend sequences, and making the unaligned files easily \DIFdelbegin \DIFdel{available to users,
researchers can compare if applying any alternate alignment tool of their choice
affects }\DIFdelend \DIFaddbegin \DIFadd{accessible and reusable,
Physcraper facilitates the exploration of alternative aligning tools to determine their effect on phylogenetic }\DIFaddend inferences.
Once sequences are aligned, \DIFdelbegin \DIFdel{they can apply and compare inferences from }\DIFdelend \DIFaddbegin \DIFadd{users can apply practically }\DIFaddend any phylogenetic
software \DIFaddbegin \DIFadd{and compare results using Physcraper tools in a reproducible framework}\DIFaddend .
...
\DIFdelbegin \DIFdel{Unlike phylogenetic placement approaches \mbox{%DIFAUXCMD
\cite{berger_performance_2011, matsen_pplacer_2010}}\hspace{0pt}%DIFAUXCMD
,
which add new taxa without modifying the input phylogeny, Physcraper estimates all
the relationships anew in the context of the new data.
PUMPER \mbox{%DIFAUXCMD
\cite{izquierdo2014pumper} }\hspace{0pt}%DIFAUXCMD
shares these conceptual strengths, but is no
...
%DIF >  Add citation of good things about thinning alignments?
...
Accidentally including paralogs as homologs is a \DIFdelbegin \DIFdel{risk in phylogenetic analyses}\DIFdelend \DIFaddbegin \DIFadd{known risk of assembling a dataset for phlyogenetic analysis}\DIFaddend ,
and can be more prevalent in \DIFdelbegin \DIFdel{automated analyses }\DIFdelend \DIFaddbegin \DIFadd{automatically assembled datasets }\DIFaddend than in manually curated \DIFdelbegin \DIFdel{analyses}\DIFdelend \DIFaddbegin \DIFadd{ones}\DIFaddend .
We provide users with several tools to \DIFdelbegin \DIFdel{try to }\DIFdelend assess homology of \DIFdelbegin \DIFdel{their }\DIFdelend aligned sequences.
...
OpenTree's conflict analysis tool \DIFaddbegin \DIFadd{implemented in Physcraper }\DIFaddend informs the users of whether their phylogeny contains major
...
\DIFdelbegin \DIFdel{Initial ideas about the data are constantly changed by results from new }\DIFdelend \DIFaddbegin \DIFadd{Scientific understanding is constantly challenged and reframed by new data and }\DIFaddend analyses.
Physcraper provides a framework for reproducible phylogenetics that has the
potential to consistently \DIFdelbegin \DIFdel{provide context for these ideas,
highlighting the importance of data sharing and open science }\DIFdelend \DIFaddbegin \DIFadd{contextualize new knowledge in the light of previous understanding,
showcasing the utility and importance of good data sharing practices and open research }\DIFaddend for phylogenetics, biology and science.
...
Code and datasets \DIFdelbegin \DIFdel{generated and analysed during the current }\DIFdelend \DIFaddbegin \DIFadd{developed and analysed for this }\DIFaddend study are available \DIFdelbegin \DIFdel{in
the repositories }\DIFdelend \DIFaddbegin \DIFadd{at
the following GitHub repositories: }\DIFaddend ``physcraper" \DIFdelbegin \DIFdel{containing }\DIFdelend \DIFaddbegin \DIFadd{-- contains }\DIFaddend the source code,
\href{https://github.com/McTavishLab/physcraper}{https://github.com/McTavishLab/physcraper};
 ``physcraperex" \DIFdelbegin \DIFdel{containing }\DIFdelend \DIFaddbegin \DIFadd{-- contains }\DIFaddend the examples,
\href{https://github.com/McTavishLab/physcraperex}{https://github.com/McTavishLab/physcraperex};
and, ``physcraper\_ms" \DIFdelbegin \DIFdel{containing }\DIFdelend \DIFaddbegin \DIFadd{-- contains }\DIFaddend this reproducible manuscript,
...
\DIFaddbegin \DIFadd{The funding body did not play any roles in the design of the study and collection,
...
All authors contributed to the manuscript\DIFdelbegin \DIFdel{and gave final approval }\DIFdelend \DIFaddbegin \DIFadd{, and have read and approved the final manuscript }\DIFaddend for publication.


\section*{Acknowledgements}
We thank the members of the \DIFaddbegin \DIFadd{current }\DIFaddend OpenTree development team \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{(MH, KC, BR, and JA),
and }\DIFaddend the ``short bar" Science
and Engineering Building 1, UCM, \DIFaddbegin \DIFadd{Blois-McTavish }\DIFaddend joint lab paper discussion group for valuable
...
\begin{figure}[h!]
  ...
  black and red, respectively. \DIFaddbeginFL \DIFaddFL{First, }\DIFaddendFL Physcraper obtained sequences from the GenBank
  database via local BLAST of all sequences \DIFdelbeginFL \DIFdelFL{from }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{in }\DIFaddendFL the Gottlieb2005 \DIFaddbeginFL \DIFaddFL{original alignment of the }\DIFaddendFL internal transcribed
  spacer DNA region (ITS)\DIFdelbeginFL \DIFdelFL{original alignment}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{. Then}\DIFaddendFL , \DIFaddbeginFL \DIFaddFL{it
  }\DIFaddendFL filtered \DIFdelbeginFL \DIFdelFL{them }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the obtained sequences }\DIFaddendFL following criteria described in section ``DNA sequence search and
  filtering", \DIFaddbeginFL \DIFaddFL{and }\DIFaddendFL aligned them to the original \DIFaddbeginFL \DIFaddFL{Gottlieb2005 }\DIFaddendFL alignment using MUSCLE\DIFaddbeginFL \DIFaddFL{. Finally}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{and }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Physcraper }\DIFaddendFL performed a
  ...
  blue branches are in agreement between the updated phylogeny and the \DIFdelbeginFL \DIFdelFL{syntehtic }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{synthetic }\DIFaddendFL subtree, and
