...
%DIF ADD sources/bmc_template_submission/bmc_article.tex                         Fri Apr 23 12:57:18 2021
%%                                      %
...
with the main goal of building on published alignments and extending }\DIFdelend \DIFaddbegin \DIFadd{.
Physcraper's main goal is to build upon published alignments to extend }\DIFaddend existing
phylogenetic inferences with more data and taxa\DIFaddbegin \DIFadd{, which improve
...
}\DIFaddend 90,000 are supported by \DIFdelbegin \DIFdel{phylogenies }\DIFdelend \DIFaddbegin \DIFadd{phylogenetic data }\DIFaddend - the remaining 1.4 million taxa are placed
...
can be leveraged as seeds to mine molecular databases, and as \DIFdelbegin \DIFdel{``}\DIFdelend \DIFaddbegin \DIFadd{"}\DIFaddend jump-start\DIFdelbegin \DIFdel{'' }\DIFdelend \DIFaddbegin \DIFadd{" }\DIFaddend alignments
...
\DIFaddbegin \DIFadd{Physcraper is implemented with Python and can also be run on the command line.
It currently consists of 12 modules.
%DIF > % , classes, methods and functions.%%
For testing and improving Python code’s syntax quality of the Phsycraper code, we
used the Pylint software following instructions from their website \mbox{%DIFAUXCMD
\citep{pylintWeb}
 }\hspace{0pt}%DIFAUXCMD
and manual \mbox{%DIFAUXCMD
\citep{pylintManual}}\hspace{0pt}%DIFAUXCMD
, with a "".pylintrc" configuration file.
}

\DIFadd{We improved code's syntax on modules with low Pylint scores and fixed code errors by
...
by using its check-disabling system, and are explained in the code.
...
\DIFadd{We pruned 9 out of the 41 original tips of the Gottlieb et al. tree, corresponding
...
\DIFadd{These sequences do appear in the Physcraper results in the list of matches from GenBank which did not fit the sequence length cutoffs set in the configuration.
...
\DIFadd{We encourage users to look at this file, and potentially modify their sequence length cutoff parameters based on the excluded sequences.
...
the effect of missing data can be enigmatic, and interact with the true phylogenetic relationships for the data set at hand.
...
\DIFadd{Organellar genome sequences, such as chloroplasts and mitochondria will also generally be excluded from automatic addition based on length cutoffs.
...
\DIFadd{Single gene phylogenies with very high numbers of taxa may lack sufficient signal for accurate resolution \mbox{%DIFAUXCMD
...
\DIFadd{Most of the default thresholds parameter values and software choices in the Physcraper pipeline are arbitrary.
...
show that when applying coalescent models, there is more information in two genes of 300 bp each than in 600 independent sites
...
\DIFaddbegin \DIFadd{Taxonomic links, and comparisons to existing published phylogenies in the OpenTree data store can also help flag paralogous sequences.
...
%DIF > This suggests that the samples might represent two ifferent species, but further analyses should be conducted
A visual comparison
