...
%DIF ADD sources/bmc_template_submission/bmc_article.tex                         Thu Apr 29 00:01:40 2021
%%                                      %
...
}\DIFdelend \DIFaddbegin \DIFadd{single locus alignment and corresponding phylogeny
...
Taxon names in the input \DIFdelbegin \DIFdel{tree }\DIFdelend \DIFaddbegin \DIFadd{phylogeny }\DIFaddend must be standardized to \DIFdelbegin \DIFdel{OpenTree taxonomy
\mbox{%DIFAUXCMD
...
Updated and original \DIFdelbegin \DIFdel{tree }\DIFdelend \DIFaddbegin \DIFadd{phylpogeny }\DIFaddend are compared with Robinson-Foulds weighted and unweighted
...
\DIFdelbegin \DIFdel{reveals }\DIFdelend \DIFaddbegin \DIFadd{revealed }\DIFaddend that there are several published phylogenies showing relationships within
...
and alignment as Physcraper inputs to update relationships in the genus }\textit{\DIFadd{Ilex}}\DIFadd{.
Currently being the best sampled and most recent phylogenetic hypothesis for the genus,
we used the Yao2020 phylogeny as a golden standard to compare our results and verify
Physcraper's performance.
}\DIFaddend 

We ran Physcraper on a laptop Linux computer to update an internal transcribed
spacer DNA region (ITS) alignment \DIFaddbegin \DIFadd{that was used to construct the phylogeny }\DIFaddend from \cite{gottlieb2005molecular},
...
\cite{yao2020phylogeny}}\hspace{0pt}%DIFAUXCMD
. }\DIFdelend \DIFaddbegin \DIFadd{additional }\textit{\DIFadd{Ilex}} \DIFadd{taxa in the Physcraper updated phylogeny (Figure 2).
}\DIFaddend 
...
\DIFadd{To test the accuracy of Physcraper we performed a verification test in which
