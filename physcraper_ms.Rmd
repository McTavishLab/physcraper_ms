...
It is well known that GenBank [@benson2000genbank; @wheeler2000database] holds enormous amounts of genetic data, and it continues to grow.
...
- The study tree should be in the OToL store [phylesystem; @mctavish2015phylesystem]. The main
  reason for this is that trees in OToL have a set of user defined characteristics
  that are essential for automatizing the phylogeny update process. The most relevant of these being the definition of ingroup and outgroup.
  A user can choose from a variety of published trees supporting any node of the synthetic OToL. If the tree you are
  interested in is not in there, you should upload it via OToL's curator tool (<https://tree.opentreeoflife.org/curator).
...
- A ".tre" and ".aln" files are generated and saved as input for a `physcraper` run.
...
- The next step is to identify and validate the search taxon. This must be a taxon (a named clade)
  on the NCBI taxonomy. It will be used to constraint the DNA sequence search on
  the nucleotide database within that taxonomic group.
  By default, the search taxon is the Most Recent Common Ancestor (MRCA) of the
  matched taxa belonging to the ingroup, that is also a named clade in the NCBI taxonomy. This is refered to as the Most Recent Common
  Ancestral Taxon (MRCAT; also reffered the Least Inclusive Common Ancestral Taxon - LICA).
  The MRCAT can be different from the phylogenetic MRCA when the latter is an unnamed clade.
  ...
  In the case that only one taxon from the ingroup is matched in both the tree and alignment, the MRCAT
  for that single taxon would be determined as ***HOW?***
  By default, the set of taxon names that are searched for their MRCAT to be used as search taxon belong to the ingroup defined by the curator in the OToL phylesystem.
  <!-- ***Physcraper does not take into account the focal clade from Open Tree. Because that is arbitrarily assigned or not by a curator. The focal clade is a search MRCA more than a taxonomic MRCA, physcraper's default is to get the ingroup from Otol, which is not the focal group.-->
...
- A pairwise all-against-all BLAST search is performed. This means that each sequence
  ...
  along with their corresponding ide numbers (accesion numbers in the case of the GenBank database). This information will be
...
- Among the remaining filtered sequences, there are usually several exemplars per taxon.
  ...
  <!--***How does pyphlawd, or phylota does the exemplar per species choosing?***-->
