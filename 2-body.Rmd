...
Public biological databases provide an amazing resource for evolutionary estimation. By linking molecular data, available from database such as the GenBank [@benson2000genbank; @wheeler2000database], to alignments and phylogenies, available in
the TreeBASE repository [@piel2009treebase] and the Open Tree of Life datastore [@mctavish2015phylesystem], we can place new biological data in evolutionary context.
However, it has been challenging to connect new sequence data with existing estimates of evolutionary relationships. 
...
Automated approaches often to suffer from errors in homology inference that are rapidly caught by human oversight. ()
...
Species tree reconstructions from multi-gene data sets taking into account the multispecies coalescent model are considered the gold standard for inferring species relationships.  \comment(Citations... I think there are some Scott Edwards papers...https://www.pnas.org/content/109/37/14942, maybe Roajs et al...)
...
- In order to take advantage of the OpenTree tools, the input tree needs to be stored in the
  ...
  A user can choose from among the `r rotl::tol_about()$num_source_trees` published trees supporting the resolved nodes of the synthetic tree in the OpenTree website ([See OpenTree's website about](https://tree.opentreeoflife.org/about/synthesis-release/v12.3)). If the published tree you are interested in updating is not in there, you can upload it via OpenTree's curator tool ([www.opentreeoflife.org/curator](https://tree.opentreeoflife.org/curator).
...
- The next step is to identify the search taxon within the reference taxonomy. The search taxon will be used to constraint the DNA sequence search on the nucleotide database within that taxonomic group. Because we are using the NCBI nucleotide database, by default the reference taxonomy is the NCBI taxonomy. The search taxon can be determined by the user. If none is provided, then the search taxon is identified as the Most Recent Common Ancestor (MRCA) of the
  matched taxa belonging to the ingroup in the OPenTree synthetic tree, that is also a named clade in the reference taxonomy. \comment(hmm, a figure might be good for this. I can work on one!) This is known as the Most Recent Common
  Ancestral Taxon (MRCAT; also referred in the literature as the Least Inclusive Common Ancestral Taxon - LICA) \comment(prob need a cite here).
  The MRCAT can be different from the phylogenetic MRCA when the latter is an unnamed clade in the reference taxonomy.
  To automatically identify the MRCAT of a group of taxon names, we make use of the OpenTree taxonomy tool (<https://github.com/OpenTreeOfLife/germinator/wiki/Taxonomy-API-v3#mrca>). I FORGOT TO ASK YOU ABOUT THIS CITATION! \comment( it's here! https://bdj.pensoft.net/article/12581/instance/3545675/)
...
- Users can choose to perform a more "cycles" of sequence similarity search, by blasting the newly found sequences. This can be done iteratively, but by default only sequences in the checked alignment are blasted. I ALSO FORGOT TO ASK: ***Is there an argument to control the number of cycles of blast searches with new sequences?*** \comment(there's only two options - one cycle or, -r where it runs until it doesn't find anything new)
- Accepted sequences are downloaded in full, and stored as a local database in a directory that is globally accessible (default to physcraper/taxonomy), so they are accessible for further runs.
