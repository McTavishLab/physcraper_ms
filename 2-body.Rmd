...
<!-- However, it has not been added to OpenTree's synthtetic tree yet. -->
This makes it a great case to highlight the functionalities of Physcraper and assess its limitations.
<!-- we know that the sequences of the Yao2020 tree have been made available on the GenBank database. Hence, we expect that updating the Gottlieb2005 tree should at least recover the same species sampled in the Yao2020 tree. -->
...
Figure \@ref(fig:results) shows results from a Physcraper analysis applied to an alignment of internal transcribed spacer DNA regions (ITS) from @gottlieb2005molecular. Physcraper ran on a local BLAST database, on a laptop Linux computer for 19hrs 45min to finish BLAST and RAxML analyses. Bootstrap analyses took an additional 13hrs.
The updated Gottlieb2005 tree contains all 41 distinct taxa from the original study plus 231 new tips, contributing phylogenetic data to 84 additional taxa within *Ilex*. The tree is 99% resolved, however, a large portion of branches are negligibly small (133 branches < 0.00001 substitution rate units), appearing as unresolved nodes (Fig. \@ref(fig:results)).
<!-- ADD percentage of nodes with bootsrap values above 95%  and below 50%?-->
...
`r 135-68` are also in updated Gottlieb2005.
...
```{r results, echo=FALSE, fig.cap="A phylogenetic tree is the main result of a Physcraper analysis.", out.width = '100%', fig.retina= 2}
...
Figure \@ref(fig:results) caption continued: The tree results from updating an alignment that was used to generate a tree of 48 tips of the genus Ilex, published in Gottlieb et al. 2005 (black tips). Physcraper added 231 new sequences (red tips) corresponding to 84 distinct taxa that were not sampled in the original alignment. Physcraper obtained sequences from the GenBank database via BLAST, filtered them following specified criteria, aligned them to the original alignment using MUSCLE and performed a phylogenetic reconstruction using RAxML, as well as a conflict analysis using the OpenTree tools.
