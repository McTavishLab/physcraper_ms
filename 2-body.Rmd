...
- A constrained BLAST search is performed, in which each sequence
  ...
  along with their corresponding identification numbers, i.e., their GenBank accession numbers, which will be used later to get the whole sequence from the GenBank database into a dedicated library within the "physcraper" folder, allowing for secondary analyses to run significantly faster.
- BLAST result sequences will be discarded if they fall outside a default minimum and maximum length of 80% and 120%, respectively, of the average length without gaps of sequences in the checked alignment.
