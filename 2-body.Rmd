...
To illustrate the utility of Physcraper we propose a scenario in which a user is interested in a particular group, e.g., the genus *Ilex*, which encompasses between 400-700 living species, and is the only extant clade within the family Aquifoliaceae, order Aquifoliales of flowering plants, commonly known as "hollies".
...
A tutorial as well as illustrated examples of functions implemented on each step of the framework are available in Physcraper's documentation website.
...
```{r ilex-updated-tree, echo=FALSE, fig.cap="A phylogenetic tree is the main result of a Physcraper analysis. The tree results from updating an input tree of 48 tips belonging to the genus Ilex published in Gottlieb et al. 2005 (black tips) with 222 new sequences (red tips) corresponding to 78 distinct taxa that were not sampled in the original alignment. Physcraper obtained sequences from the GenBank database via BLAST, filtered them following specified criteria, aligned them to the original alignment using MUSCLE and performed a phylogenetic reconstruction using RAxML as well as a conflict analysis", out.width = '100%', fig.retina= 2}

knitr::include_graphics("docs/figs/ilex-plot-tips-and-branches-1.png")

```

The Gottlieb2005 updated tree has 48 original tips, representing 45 distinct taxa, and 231 new tips representing 84 distinct taxa.
