...
Phylogenetic estimates of evolutionary relationships capture the shared history of living organisms, and provide context for biological observations.
Public biological databases provide an amazing resource for evolutionary estimation, but there is a lot of sequence data available that has never been incorporated into any phylogenetic estimates.

GenBank contains over 217 million DNA sequences [(release number 238; June 15, 2020)](https://ftp.ncbi.nih.gov/genbank/gbrel.txt).
These sequences are estimated to have the potential to resolve evolutionary relationships of most of the 241 000 distinct taxa in GenBank [about `r round(236023/240708, digits=4)*100`% of taxa in the NCBI taxonomy; @sanderson2008phylota].
...
Yet, publicly available estimates of phylogenetic relationships still only about 100 000 taxa [@piel2009treebase; @hinchliff2015synthesis; @opentreeoflife_2019_synth].
These data comprise less than one third of the taxonomic diversity for which sequence data was available a decade ago!

Some of this discrepancy between data availability and phylogenetic estimates is because many phylogenies which are generated and published are not publicly shared in an accesible way [@drew2013lost; @magee2014dawn; @mctavish2018bioessay]. However, there is also a lag between the amount of new DNA data generated and the analysis of these data in a phylogenetic context. Connecting new sequence data with existing estimates of evolutionary relationships is a challenge.

We address this gap by extending existing phylogenetic estimates with publicly available sequence data.  By using a starting tree and single locus alignment, Physcraper, takes advantage of existing research, and extends trees for of loci that taxon specialists have assessed and deemed appropriate for the phylogenetic scope.
...
Even when the same sequencing approach is applied, such as RadSeq phylogenetic distance can cause allelic dropout at deeper divergences [@eaton2016]
...
ELABORATE MORE ON THIS, USE LESS CASUAL LANGUAGE: This is an effort to keep on directing ourselves towards a fully reproducible workflow in phylogenetics.
